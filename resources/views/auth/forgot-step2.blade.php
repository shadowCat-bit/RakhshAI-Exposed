...
<div class="main-inner auth-page-container">
    <main class="main-sec auth-page forget-step-2" style="padding-bottom: 50px">
  <div class="container">
      ...
      <div class="col-sm-10 col-md-7 col-lg-5">
          ...
          <div class="card-header text-center">
              <h1 class="page-title">تایید تغییر رمز عبور</h1>
          </div>
          @if($errors->has('msg'))
          ...
          @endif
          <div class="card-body">
              <form action="{{route('forgot-password.process')}}" method="post" id="loginForm">
                @csrf
                <input type="hidden" name="recaptcha" id="recaptcha">
                <input type="hidden"name="mobile" value="{{request('mobile')}}">
                <div class="mb-3">
                  <label for="mobile" class="form-label">لطفا پس از وارد کردن رمز عبور جدید خود، کد ارسال شده به شماره <span class="verify-number">{{request('mobile')}}</span> را وارد نمایید</label>
                </div>
                <div class="mb-3 position-relative toggle-password">
                  <label for="password" class="form-label">رمز عبور جدید:</label>
                  <input autofocus type="password" class="form-control pass-input input-s convert-to-en-num" id="password" name="password" minlength="4" required>
                ...
                <label for="verify_code" class="form-label">کد تایید  <span class="verify-explain">(ارسال شده به موبایل شما)</span>:</label>
                <div class="input-group mb-3">
                    <input type="text" class="form-control input-s convert-to-en-num" id="code" name="verify_code" placeholder="" maxlength="4" length="4" required>
                </div>

                <div class="send-code-again">
                    <div id="countdown"></div>
                    <a id="new-code-button" href="{{url("auth/forgot-password/confirm?mobile=$mobile&send_code=yes")}}" class="text-style-1 decoration-none" type="button">ارسال مجدد کد</a>
                </div>


                  <button type="submit" class="btn-gr style-5 btn-submit w-100 mt-3 mx-0">
                      <span class="submit-btn-text">تایید تغییر رمز عبور</span>
...
<script>
         grecaptcha.ready(function() {
             grecaptcha.execute('{{ config('services.recaptcha.sitekey') }}', {action: 'forgotpass_step2'}).then(function(token) {
                if (token) {
